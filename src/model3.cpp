...
//ASZ __________________________________________________
bool model3::set_twin(model3& another_m3, string prefixAct, int m345_SymFrequency, int m345_TMultiplier){
  /* This function add symetrization to computation
  */
	isTwin=true;
	anotherM3=&another_m3;
	actualPrefix=prefixAct;
	m345SymFreq = m345_SymFrequency;
	m345TMultiplier = m345_TMultiplier;

    cout << "Twin was add for model 3 with prefix "<<prefixAct<<" and symetrization frequency "<<m345_SymFrequency<<endl;
    return true;
}
...
void model3::em(int noIterations, sentenceHandler& sHandler1) {
	...
	for (int it=1; it <= noIterations; it++) {
		...
		string actPref="";
		if(actualPrefix.compare("")!=0) actPref=actualPrefix;
		else actPref=Prefix;
		tfile = actPref + ".t3." + number;
		afile = actPref + ".a3." + number;
		nfile = actPref + ".n3." + number;
		dfile = actPref + ".d3." + number;
		p0file = actPref + ".p0_3." + number;
		//    tCountTable.clear();
		dCountTable.clear();
		nCountTable.clear();
		p0_count = 0.0;
...
int model3::viterbi(int noIterationsModel3, int noIterationsModel4,
	...
	for (unsigned int it=1; it < trainingString.length(); it++) {
		...
		string d4file2;
		{
			...
			tfile = actPref + ".t3." + number;
			tfile_actual = actPref + ".actual.t3." + number;
			afile = actPref + ".a3." + number;
			nfile = actPref + ".n3." + number;
			nfile_actual = actPref + ".actual.n3." + number;
			dfile = actPref + ".d3." + number;
			d4file = actPref + ".d4." + number;
			d4file2 = actPref + ".D4." + number;
			d5file = actPref + ".d5." + number;
			alignfile = actPref + ".A3." + number;
			test_alignfile = actPref + ".tst.A3." + number;
			p0file = actPref + ".p0_3." + number;
		}
		// clear count tables 
		//    tCountTable.clear();
...
//		dump_files = true;
		...
		nCountTable.normalize(nTable, &Elist.getVocabList());
		 ...
		 isEnd=true;
//_____symetrization______________________________

		        if(isTwin && symNumber==m345SymFreq){
		        	cout<<"Running symetrization from "<<fromModel<<" to "<<toModel<<", iteration "<<it<<" with t multipler "<<m345TMultiplier<<endl;
		        	symNumber=0;//number of iteration after symetrization

		        	while(((*anotherM3).isEnd==false || (*anotherM3).iterationNumber!=iterationNumber) && symetrizeT==false) {
		        		sleep(1);
		        	}
		        	...
		        	if(symetrizeT){
		        		...
		        		(*anotherM3).isEnd=false;
		        		isEnd=false;
		        		//______________symetrization of t files...____________________
		        		/*string Tfile = Prefix + ".t." + number ;
		                tTable.printProbTable((Tfile+"_EF_before").c_str(),Elist.getVocabList(),
		                Flist.getVocabList(),OutputInAachenFormat);
		                (*anotherM3).tTable.printProbTable((Tfile+"_FE_before").c_str(),Flist.getVocabList(),
		                Elist.getVocabList(),OutputInAachenFormat);*/
		        		...
		        		while(sHandler1.getNextSentence(sent)){
		        			...
		        			for(j=1; j <= m; j++)
		        				for(i = 0; i <= l; i++)
		        				{
		        					...
		        					PROB pr2 = (*anotherM3).tTable.getProb(fs[j],es[i]);
		        					if(pr1!=PROB_SMOOTH && pr2!=PROB_SMOOTH) {
		        						PROB srednia = ((pr1+pr2)/2);

		        						tTable.insert(es[i],fs[j], 0.0, srednia);
		        						(*anotherM3).tTable.insert(fs[j],es[i], 0.0, srednia);

		        					}
		        					else{
		        						if(pr1 > PROB_SMOOTH) tTable.insert(es[i],fs[j], 0.0, pr1/m345TMultiplier);
		        						if(pr2 > PROB_SMOOTH) (*anotherM3).tTable.insert(fs[j],es[i], 0.0, pr2/m345TMultiplier);

		        						/*tTable.erase(es[i],fs[j]);
		        						(*anotherM3).tTable.erase(fs[j],es[i]);*/
		        					}
		        					if(l==100 && m==100){
		        						double a = aTable.getValue(i,j,100,100);
		        						double b = (*anotherM3).aTable.getValue(j,i,100,100);
		        						if(a>0.000001 && b>0.000001) {
		        							//cout<<i<<":"<<j<<":"<<a<<", "<<b<<endl;
		        							double sredniaA = (a+b)/2;
		        							aTable.setValue(i,j, 100, 100,sredniaA) ;
		        							(*anotherM3).aTable.setValue(j,i, 100, 100,sredniaA);
		        						}
		        		...
		        		/*tTable.printProbTable((Tfile+"_EF_after").c_str(),Elist.getVocabList(),
		                Flist.getVocabList(),OutputInAachenFormat);
		                (*anotherM3).tTable.printProbTable((Tfile+"_FE_after").c_str(),Flist.getVocabList(),
		                Elist.getVocabList(),OutputInAachenFormat);*/
