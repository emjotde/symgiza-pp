...
bool model2::set_twin(model2& another_m2, string prefixAct, int Model2_SymFrequency, int m2_TMultiplier){
  /* This function add symetrization to computation
  */
	isTwin=true;
	anotherModel2=&another_m2;
	actualPrefix=prefixAct;
	m2SymFreq = Model2_SymFrequency;
	m2TMultiplier = m2_TMultiplier;

    cout << "Twin was add for model 2 with prefix "<<prefixAct<<" and symetrization frequency "<<Model2_SymFrequency<<endl;
    return true;
}
...
int model2::em_with_tricks(int noIterations,bool dumpCount, 
    ...
    for(int it=1; it <= noIterations ; it++){
        ...
        tfile = actPref + ".t" + shortModelName + "." + number ;
        afile = actPref + ".a" + shortModelName + "." + number ;
        alignfile = actPref + ".A" + shortModelName + "." + number ;
        test_alignfile = actPref + ".tst.A" + shortModelName + "." + number ;
        aCountTable.clear();
        initAL();
        ...
        isEnd=true;
               ...
               if(isTwin && symNumber==m2SymFreq){
               	cout<<"Running symetrization for model 2, iteration "<<noIterations<<" with t multipler "<<m2TMultiplier<<endl;
               	symNumber=0;//number of iteration after symetrization

               	while(((*anotherModel2).isEnd==false || (*anotherModel2).iterationNumber!=iterationNumber) && symetrizeT==false) {
               		sleep(1);
               		}
               				 ...
               				 Elist.getVocabList(),OutputInAachenFormat);*/
               		 		        ...
               		 		        while(sHandler1.getNextSentence(sent)){
               		 		                	...
               		 		                	for(i = 0; i <= l; i++)
               		 		                {
               		 		                		...
               		 		                		PROB pr2 = (*anotherModel2).tTable.getProb(fs[j],es[i]);
               		 		                		if(pr1!=PROB_SMOOTH && pr2!=PROB_SMOOTH) {
               		 		                			PROB srednia = ((pr1+pr2)/2);

               		 		                			tTable.insert(es[i],fs[j], 0.0, srednia);
               		 		                			(*anotherModel2).tTable.insert(fs[j],es[i], 0.0, srednia);

               		 		                		}
               		 		                		else{
               		 		                		if(pr1 > PROB_SMOOTH) tTable.insert(es[i],fs[j], 0.0, pr1/m2TMultiplier);
               		 		                		if(pr2 > PROB_SMOOTH) (*anotherModel2).tTable.insert(fs[j],es[i], 0.0, pr2/m2TMultiplier);

               		 		                			/*tTable.erase(es[i],fs[j]);
               		 		                			(*anotherModel2).tTable.erase(fs[j],es[i]);*/
               		 		                		}
               		 		                		if(l==100 && m==100){
               		 		                		double a = aTable.getValue(i,j,100,100);
               		 		                		double b = (*anotherModel2).aTable.getValue(j,i,100,100);
               		 		                		if(a>0.000001 && b>0.000001) {
               		 		                			//cout<<i<<":"<<j<<":"<<a<<", "<<b<<endl;
               		 		                		double sredniaA = (a+b)/2;
               		 		                		aTable.setValue(i,j, 100, 100,sredniaA) ;
               		 		                		(*anotherModel2).aTable.setValue(j,i, 100, 100,sredniaA) ;
               		 		                		}
               		 		                		}
               		 		        ...
               		 		        /*string Tfile = Prefix + ".t" + shortModelName + "." + number ;
               		 		        tTable.printProbTable((Tfile+"_EF").c_str(),Elist.getVocabList(),
               		 		                                              Flist.getVocabList(),OutputInAachenFormat);
               		 		       (*anotherModel2).tTable.printProbTable((Tfile+"_FE").c_str(),Flist.getVocabList(),
               		 		                                              Elist.getVocabList(),OutputInAachenFormat);*/
               		 		        //_____________________________________________________________


               		 (*anotherModel2).symetrizeT=true;
