...
//    dump_files = true;
    ...
    alignfile = actPref + ".A" + shortModelName + "." + number + ".part" ;
...
, bool dumpCount ,  const char* dumpCountName, bool useString)  // If specified, then will dump files before last iteration
{
	...
	//dump_files = true;
        ...
        alignfile = actPref + ".A" + shortModelName + "." + number+".part0" ;
        ...
        if(isTwin && symNumber==m1SymFreq){
        	cout<<"Running symetrization for model 1, iteration "<<noIterations<<" with t multipler "<<m1TMultiplier<<endl;
        	symNumber=0;//number of iteration after symetrization
        	while(((*anotherModel1).isEnd==false || (*anotherModel1).iterationNumber!=iterationNumber) && symetrizeT==false) {
        		sleep(1);
        		}
        	if(symetrizeT){
        		symetrizeT=false;
        	} else{
        		(*anotherModel1).isEnd=false;
        		 isEnd=false;
        		 ...
        		 (*anotherModel1).tTable.printProbTable((Tfile+"_FE_before").c_str(),Flist.getVocabList(),
        				 Elist.getVocabList(),OutputInAachenFormat);*/
        		 		        ...
        		 		        while(sHandler1.getNextSentence(sent)){
        		 		                	...
        		 		                	for(i = 0; i <= l; i++)
        		 		                {
        		 		                		...
        		 		                		if(pr1!=PROB_SMOOTH && pr2!=PROB_SMOOTH) {
        		 		                			...
        		 		                			//tTableF.erase(es[i],fs[j]);
        		 		                			PROB srednia = (pr1+pr2)/2;

        		 		                			tTable.insert(es[i],fs[j], 0.0, srednia);
        		 		                			(*anotherModel1).tTable.insert(fs[j],es[i], 0.0, srednia);
        		 		                			//cout << "tEF:"<<es[i]<<":"<<fs[j]<<":"<<tTable.getProb(es[i],fs[j])<<endl;
        		 		                			//cout << "tFE:"<<es[i]<<":"<<fs[j]<<":"<<(*anotherModel1).tTable.getProb(es[i],fs[j])<<endl;

        		 		                		}
        		 		                		else{
        		 		                			if(pr1 > PROB_SMOOTH) tTable.insert(es[i],fs[j], 0.0, pr1/m1TMultiplier);
        		 		                			if(pr2 > PROB_SMOOTH) (*anotherModel1).tTable.insert(fs[j],es[i], 0.0, pr2/m1TMultiplier);

        		 		                			/*tTable.erase(es[i],fs[j]);
        		 		                			(*anotherModel1).tTable.erase(fs[j],es[i]);*/
        		 		                		}
        		 		                }
        		 		        }
        		 		        ...
        		 		        tTable.printProbTable((Tfile+"_EF").c_str(),Elist.getVocabList(),
        		 		                                              Flist.getVocabList(),OutputInAachenFormat);
        		 		       (*anotherModel1).tTable.printProbTable((Tfile+"_FE").c_str(),Flist.getVocabList(),
        		 		                                              Elist.getVocabList(),OutputInAachenFormat);*/
...
bool model1::set_twin(model1& another_m1, string prefixAct, int Model1_SymFrequency, int m1_TMultiplier){
  /* This function add symetrization to computation
  */
	isTwin=true;
	anotherModel1=&another_m1;
	actualPrefix=prefixAct;
	m1SymFreq = Model1_SymFrequency;
	m1TMultiplier=m1_TMultiplier;

    cout << "Twin was add for model 1 with prefix "<<prefixAct<<" and symetrization frequency "<<Model1_SymFrequency<<endl;
    return true;
}
