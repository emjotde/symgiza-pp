...
bool hmm::set_twin(hmm& another_mh, string prefixAct, int hmm_SymFrequency, int hmm_TMultiplier){
  /* This function add symetrization to computation
  */
	isTwin=true;
	anotherHMM=&another_mh;
	actualPrefix=prefixAct;
	hmmSymFreq = hmm_SymFrequency;
	hmmTMultiplier = hmm_TMultiplier;

    cout << "Twin was add for hmm with prefix "<<prefixAct<<" and symetrization frequency "<<hmm_SymFrequency<<endl;
    return true;
}
...
int hmm::em_with_tricks(int noIterations,bool dumpCount, 
    ...
    for(int it=1; it <= noIterations ; it++){
       ...
       // acfile = Prefix + ".ac" + shortModelName + "." + number ;
        afileh = actPref + ".h" + shortModelName + "." + number ;
        ...
        if(isTwin && symNumber==hmmSymFreq){
        	cout<<"Running symetrization for hmm, iteration "<<noIterations<<" with t multipler "<<hmmTMultiplier<<endl;
        	symNumber=0;//number of iteration after symetrization

        	while(((*anotherHMM).isEnd==false || (*anotherHMM).iterationNumber!=iterationNumber) && symetrizeT==false) {
        		sleep(1);
        	}
        	...
        	if(symetrizeT){
                ...
                (*anotherHMM).tTable.printProbTable((Tfile+"_FE_before").c_str(),Flist.getVocabList(),
        		...
        		while(sHandler1.getNextSentence(sent)){
        			...
        			for(j=1; j <= m; j++)
        				for(i = 0; i <= l; i++)
        				{
        					...
        					PROB pr2 = (*anotherHMM).tTable.getProb(fs[j],es[i]);
        					if(pr1!=PROB_SMOOTH && pr2!=PROB_SMOOTH) {
        						PROB srednia = ((pr1+pr2)/2);

        						tTable.insert(es[i],fs[j], 0.0, srednia);
        						(*anotherHMM).tTable.insert(fs[j],es[i], 0.0, srednia);

        					}
        					else{
        						if(pr1 > PROB_SMOOTH) tTable.insert(es[i],fs[j], 0.0, pr1/hmmTMultiplier);
        						if(pr2 > PROB_SMOOTH) (*anotherHMM).tTable.insert(fs[j],es[i], 0.0, pr2/hmmTMultiplier);

        						/*tTable.erase(es[i],fs[j]);
        						(*anotherHMM).tTable.erase(fs[j],es[i]);*/
        					}
        					if(l==100 && m==100){
                ...
                Flist.getVocabList(),OutputInAachenFormat);
                (*anotherHMM).tTable.printProbTable((Tfile+"_FE").c_str(),Flist.getVocabList(),
                Elist.getVocabList(),OutputInAachenFormat);*/
...
int multi_thread_em(int noIter, int noThread, hmm* base){
    ...
    for(i=1;i<=noIter;i++){
        ...
        tfile = actPref + ".t" + shortModelName + "." + number ;
        afile = actPref + ".a" + shortModelName + "." + number ;
        acfile = actPref + ".ac" + shortModelName + "." + number ;
        afileh = actPref + ".h" + shortModelName + "." + number ;
